...
- The next highest-impact work is target Pi validation of the corrected any-two-button Focus routine trigger and sampled GPIO release recovery, LED output, microphone privacy rules, and installed service validation.
...
- Real GPIO input backend: `tcube-pi --backend pi` claims BCM lines with internal pull-downs through `rppal` behind the `pi-gpio` Cargo feature (target-scoped to Linux so macOS `--all-features` builds stay green). `src/hardware/gpio.rs` holds a platform-neutral, unit-tested pin map parser (`--button-gpios`, default `17,27,22,5,6` matching the five-button assembly and the GPIO17 breadboard starter), a 30 ms software debouncer, and an input pipeline that preserves immediate normal-button presses while treating sampled pin levels as authoritative. Startup-high levels seed only the debouncer and remain gesture-ineligible until observed low and pressed again; every interrupt/poll loop reconciles all five `InputPin::is_high` levels before ticking the gesture recognizer, so a release suppressed inside the debounce window cannot leave stale held state. The first eligible pair of distinct logical buttons pressed within 500 ms remains the accepted pair until reset, must stay held for 3 s, tolerates a micro-release up to 250 ms, ignores other buttons, fires once, and logs one trigger-only journald line with the pair, assembly milliseconds, and hold seconds.
...
- Target-hardware validation of the corrected any-two-button Focus routine trigger and sampled GPIO release recovery.
