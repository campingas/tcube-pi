...
- [x] Validate the new `tcube-pi.service` GPIO runtime end to end on target hardware: fresh install, reboot, GPIO17 button press to speaker audio, admin UI remap taking effect live, and installer idempotence on re-run.
...
- [ ] Validate the corrected any-two-button Focus routine trigger and sampled GPIO release recovery on the five-button target assembly.
