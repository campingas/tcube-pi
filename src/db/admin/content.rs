...
pub(crate) fn active_content_rows(
    ...
    let sql = if has_button_events {
        if language.is_some() {
            "select ci.id, ci.content_type, ci.title, ci.text, ci.language, ci.source, ci.state, ci.audio_path, ci.button_id, count(be.id) \
             ...
             group by ci.id, ci.content_type, ci.title, ci.text, ci.language, ci.source, ci.state, ci.audio_path, ci.button_id, ci.order_index \
        ...
        "select id, content_type, title, text, language, source, state, audio_path, button_id, 0 \
         from content_items \
